...
const Toast = ({ message, isVisible, onClose }: { message: string, isVisible: boolean, onClose: () => void }) => {
  ...
  return (
    <div className={`fixed rounded-xl top-6 left-1/2 transform -translate-x-1/2 bg-[#1AA033] text-white px-6 py-3 shadow-lg z-50 ${animationClass}`}>
      {message}
    </div>
  );
};
...
export default function RecipePage() {
  ...
  if (isLoading) {
    return (
      <div className="container mx-auto px-4 py-8">
        <p className="font-matina">Loading recipe...</p>
      </div>
    );
  }
  ...
  const dietaryRestrictions = recipe.tags?.filter((tag) => 
    ["Vegan", "Vegetarian", "Gluten-Free", "Dairy-Free", "Nut-Free", "Egg-Free", "Fish-Free", "Meat-Free"].includes(tag)
  ) || [];

  // Other tags
  const otherTags = recipe.tags?.filter((tag) => 
    !["Vegan", "Vegetarian", "Gluten-Free", "Dairy-Free", "Nut-Free", "Egg-Free", "Fish-Free", "Meat-Free"].includes(tag)
  ) || [];
      ...
      <div className="flex flex-col md:flex-row">
        ...
        <div className="md:w-1/2 p-6 md:p-10 bg-white">
          <div className="flex justify-between items-center mb-4">
            <h1 className="font-gaya text-3xl md:text-4xl font-bold">{recipe.title || "Untitled Recipe"}</h1>
            <button 
              onClick={toggleLike}
          ...
          <p className="font-matina text-lg mb-8">{recipe.description || recipe.cuisine || "No description available."}</p>
          ...
          <div className="grid grid-cols-2 gap-4 mb-8">
            <div className="bg-[#fff8e7] p-4 rounded-lg">
              ...
              <p className="font-matina mt-1">{recipe.cook_time_minutes ? `${recipe.cook_time_minutes} minutes` : "30 minutes"}</p>
            </div>
          ...
          {otherTags.length > 0 && (
            <div className="flex flex-wrap mb-4">
              {otherTags
                ...
                .map((tag, index) => (
                  <span key={index} className="tag-pill font-matina bg-gray-200 text-gray-800 px-3 py-1 rounded-full mr-2 mb-2">
                    {tag}
                  </span>
                ))}
            </div>
              ...
              <ul className="bg-white rounded-lg p-6 shadow-md">
                {ingredients.length > 0 ? (
                  ingredients.map((ingredient, index) => (
                    <li key={index} className="font-matina mb-3 cursor-pointer" onClick={() => toggleIngredient(index)}>
                      <div className="grid grid-cols-[auto_auto_1fr] gap-2 items-start">
                        ...
                        />
                        <div className={`font-bold whitespace-nowrap ${checkedIngredients[index] ? 'line-through text-gray-400' : ''}`}>
                          {ingredient.amount || ""}
                          {ingredient.unit ? ` ${formatUnit(ingredient.unit, ingredient.amount)}` : ""}
                        </div>
                        <div className={checkedIngredients[index] ? 'line-through text-gray-400' : ''}>
                          {ingredient.name}
                        </div>
                      </div>
                    </li>
                ...
                {instructions.length > 0 ? (
                  instructions
                    ...
                    .map((instruction, index) => {
                      ...
                      if (ingredients.length > 0) {
                        ingredients.forEach((ing) => {
                          if (ing.name) {
                            ...
                            const baseName = ingName.split(' ').slice(0, 2).join(' '); // Use first two words for matching
                      ...
                      return (
                        <li key={instruction.id || index} className="font-matina relative pl-8">
                          <span className="absolute left-0 top-0 font-gaya text-2xl font-bold">{index + 1}.</span>
